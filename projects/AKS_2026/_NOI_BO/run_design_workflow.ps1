...
if (-not $needDownload) {
  & $python "projects\AKS_2026\_NOI_BO\pipeline\ensure_design_covariates.py"
...
if ($needDownload) {
  ...
  RunChecked $python @("projects\AKS_2026\_NOI_BO\pipeline\run_download_clhs.py")
...
if (
  ...
  if (-not $ForceDownload -and -not $rawChanged -and $pcaAssetsReady) {
    ...
    try {
      RunChecked $rscript @("projects\AKS_2026\_NOI_BO\pipeline\build_clhs_pca.R")
    } finally {
...
RunChecked $python @("projects\AKS_2026\_NOI_BO\pipeline\design_clhs_scientific_smoke_test.py")
