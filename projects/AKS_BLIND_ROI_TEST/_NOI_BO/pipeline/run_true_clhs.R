...
repository_root <- normalizePath(file.path(internal_dir, "..", "..", ".."), winslash = "/", mustWork = FALSE)
application_lib <- file.path(repository_root, "_UNG_DUNG", "runtime", "R_library")
dir.create(application_lib, recursive = TRUE, showWarnings = FALSE)
.libPaths(unique(c(application_lib, .libPaths())))
...
if (!requireNamespace("clhs", quietly = TRUE) && auto_install) {
  ...
  tryCatch(
    install.packages("clhs", lib = application_lib, repos = repository,
...
if (!isTRUE(selected$record$minimum_spacing_target_met) ||
    !identical(as.numeric(selected$record$spacing_violation_pairs), 0)) {
  stop(
    "No CRAN-clhs restart met minimum_spacing_m; no official core was emitted. ",
    "Increase restarts/iterations, reduce requested sample size, or review the spacing target."
  )
}
