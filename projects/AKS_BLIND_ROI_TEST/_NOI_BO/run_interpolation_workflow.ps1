...
RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\preflight_actual.R")
...
function Certify-Workflow1PcaCopy {
  ...
  RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\verify_current_pca_provenance.R")
  RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\preflight_actual.R")
}
...
RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\diagnose_outside_covariate_support.R")
...
if ($hasSoil) {
  RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\run_prepare_soil_actual.py")
  RunChecked $rscript @("_UNG_DUNG\engine\scripts\validate_raster_schema.R", "--config", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\config\full_pc_soil.R", "--point", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\work\interpolation\sample_actual_clean.csv", "--output", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\work\interpolation\qa\raster_schema_current.json")
} else {
  Write-Host "[INFO] Khong co Soil Type; chi chay mo hinh PC." -ForegroundColor Yellow
  RunChecked $rscript @("_UNG_DUNG\engine\scripts\validate_raster_schema.R", "--config", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\config\full_pc.R", "--point", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\work\interpolation\sample_actual_clean.csv", "--output", "projects\AKS_BLIND_ROI_TEST\_NOI_BO\work\interpolation\qa\raster_schema_current.json")
}

Write-Host "[6/8] Kiem tra ket qua phan tich..." -ForegroundColor Cyan
RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\prepare_model_input.R")
...
function RunModel([string]$Indicator, [string]$ModelName, [string]$ConfigFile, [string]$OutputRoot) {
  ...
  $runName = "AKS_BLIND_ROI_TEST_${safeIndicator}_${ModelName}"
  ...
  RunChecked (RscriptPath) @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\04_mask_final_to_roi.R", "--run_dir", $expectedRun, "--output_dir", $mapOut)
...
foreach ($indicator in $filled) {
  RunModel $indicator "PC_ONLY" "projects/AKS_BLIND_ROI_TEST/_NOI_BO/config/rk_pc.R" (Join-Path $modelWork "PC_ONLY")
  if ($hasSoil) { RunModel $indicator "PC_PLUS_SOIL" "projects/AKS_BLIND_ROI_TEST/_NOI_BO/config/rk_pc_soil.R" (Join-Path $modelWork "PC_PLUS_SOIL") }
}
