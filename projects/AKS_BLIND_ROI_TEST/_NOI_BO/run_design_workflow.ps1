...
if (-not $needDownload) {
  & $python "projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\ensure_design_covariates.py"
...
if ($needDownload) {
  ...
  RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\run_download_clhs.py")
  RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\ensure_design_covariates.py")
} else {
...
if (
  ...
  if (-not $ForceDownload -and -not $rawChanged -and $pcaAssetsReady) {
    ...
    try {
      RunChecked $rscript @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\build_clhs_pca.R")
    } finally {
...
RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\prepare_clhs_soil.py")
RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\ensure_design_covariates.py")
$coverage = Get-Content -LiteralPath $coverageQa -Raw | ConvertFrom-Json
...
RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\run_design_clhs_official.py")
Write-Host "[5/5] Dang kiem tra metadata va QA khoa hoc..." -ForegroundColor Cyan
RunChecked $python @("projects\AKS_BLIND_ROI_TEST\_NOI_BO\pipeline\design_clhs_scientific_smoke_test.py")
