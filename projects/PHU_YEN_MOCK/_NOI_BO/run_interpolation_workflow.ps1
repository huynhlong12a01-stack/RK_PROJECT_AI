if ($env:RK_PHU_YEN_MOCK_SAFE_RUN -ne 'orchestrated-v1') {
  throw 'PHU_YEN_MOCK is fail-closed. Run 00_CHAY_MOCK_AN_TOAN.bat; direct interpolation is blocked.'
}
...
RunChecked $rscript @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\preflight_actual.R")
...
function Certify-Workflow1PcaCopy {
  ...
  RunChecked $rscript @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\verify_current_pca_provenance.R")
  RunChecked $rscript @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\preflight_actual.R")
}
...
RunChecked $rscript @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\diagnose_outside_covariate_support.R")
...
if ($hasSoil) {
  RunChecked $python @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\run_prepare_soil_actual.py")
  RunChecked $rscript @("_UNG_DUNG\engine\scripts\validate_raster_schema.R", "--config", "projects\PHU_YEN_MOCK\_NOI_BO\config\full_pc_soil.R", "--point", "projects\PHU_YEN_MOCK\_NOI_BO\work\interpolation\sample_actual_clean.csv", "--output", "projects\PHU_YEN_MOCK\_NOI_BO\work\interpolation\qa\raster_schema_current.json")
} else {
  Write-Host "[INFO] Khong co Soil Type; chi chay mo hinh PC." -ForegroundColor Yellow
  RunChecked $rscript @("_UNG_DUNG\engine\scripts\validate_raster_schema.R", "--config", "projects\PHU_YEN_MOCK\_NOI_BO\config\full_pc.R", "--point", "projects\PHU_YEN_MOCK\_NOI_BO\work\interpolation\sample_actual_clean.csv", "--output", "projects\PHU_YEN_MOCK\_NOI_BO\work\interpolation\qa\raster_schema_current.json")
}

Write-Host "[6/8] Kiem tra ket qua phan tich..." -ForegroundColor Cyan
RunChecked $rscript @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\prepare_model_input.R")
...
function RunModel([string]$Indicator, [string]$ModelName, [string]$ConfigFile, [string]$OutputRoot) {
  ...
  $runName = "PHU_YEN_MOCK_${safeIndicator}_${ModelName}"
  ...
  RunChecked (RscriptPath) @("projects\PHU_YEN_MOCK\_NOI_BO\pipeline\04_mask_final_to_roi.R", "--run_dir", $expectedRun, "--output_dir", $mapOut)
...
foreach ($indicator in $filled) {
  RunModel $indicator "PC_ONLY" "projects/PHU_YEN_MOCK/_NOI_BO/config/rk_pc.R" (Join-Path $modelWork "PC_ONLY")
  if ($hasSoil) { RunModel $indicator "PC_PLUS_SOIL" "projects/PHU_YEN_MOCK/_NOI_BO/config/rk_pc_soil.R" (Join-Path $modelWork "PC_PLUS_SOIL") }
}
