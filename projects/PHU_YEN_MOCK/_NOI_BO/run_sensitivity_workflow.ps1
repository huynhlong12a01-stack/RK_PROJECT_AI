...
if ($env:RK_PHU_YEN_MOCK_SAFE_RUN -ne 'orchestrated-v1') {
  throw 'PHU_YEN_MOCK is fail-closed. Run 00_CHAY_MOCK_AN_TOAN.bat; direct sensitivity execution is blocked.'
}
    ...
    foreach ($branch in $branches) {
      ...
      foreach ($indicator in $filled) {
        $safeIndicator = ($indicator -replace '[^A-Za-z0-9_-]', '_') `
          -replace '_+', '_'
        $runName = "PHU_YEN_MOCK_${safeIndicator}_$($branch.Name)_$($scenario.Id)"
...
Run-Checked $rscript.Source @(
  "projects\PHU_YEN_MOCK\_NOI_BO\pipeline\compare_sensitivity_results.R"
)
