...
def run_supervised(paths, roi_path, label_path, config, config_path, preflight_only):
    contract_path = paths.project / "MOCK_CONTRACT.json"
    if contract_path.is_file():
        contract = json.loads(contract_path.read_text(encoding="utf-8"))
        if contract.get("project_id") == "PHU_YEN_MOCK":
            return blocked(
                paths,
                "BLOCKED_MOCK_CLASSIFIER_TRAINING",
                training_eligible=False,
                reference_reuse_eligible=False,
                reason="Synthetic mock labels cannot train or evaluate a classifier.",
            )
    _, temporal = TEMPORAL.rolling_periods(config)
