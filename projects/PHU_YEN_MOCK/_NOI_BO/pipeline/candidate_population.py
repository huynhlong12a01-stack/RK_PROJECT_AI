"""Exact candidate thinning with all valid uint8 Soil Type factor levels."""
...
def _valid_block(sources, soil_src, core_mask, window):
    ...
    soil_valid = soil >= 1
    soil_nodata = getattr(soil_src, "nodata", None)
    if soil_nodata is not None and np.isfinite(soil_nodata):
        soil_valid &= soil != soil_nodata
    row0, col0 = int(window.row_off), int(window.col_off)
    core = core_mask[
        row0 : row0 + int(window.height),
        col0 : col0 + int(window.width),
    ] == 1
    valid = core & band_valid & soil_valid
...
def sample_valid_candidates(
    *, sources, soil_src, core_mask, transform, max_candidates, rng
    ...
    """Uniformly sample exact valid-population ranks in two block passes."""
    ...
    feature_chunks, coordinate_chunks, soil_chunks = [], [], []
    population_offset = 0
    ...
    return features, coordinates, soils, {
        ...
        "soil_factor_validity_rule": "all codes >=1 except raster nodata",
        "soil_factor_top_n_filter_applied": False,
    }
