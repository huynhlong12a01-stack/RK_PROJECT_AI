...
parse_args <- function(args) {
  out <- list(
    ...
    output = "agent/responses/rag_query_result.json",
    top_k = 8L
  )
  i <- 1L
  while (i <= length(args)) {
...
if (is.null(args$query) || !nzchar(trimws(args$query))) {
  stop("Missing --query. Example: Rscript scripts/rag_query_local_index.R --query \"spatial cross-validation\"", call. = FALSE)
}
