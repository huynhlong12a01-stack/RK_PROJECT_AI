...
def select_true_clhs_core(
    *,
    ...
    spacing = metadata.get("multi_start_selection") or {}
    if (
        spacing.get("minimum_spacing_target_met") is not True
        or int(spacing.get("selected_spacing_violation_pairs", -1)) != 0
    ):
        raise RuntimeError(
            "CRAN clhs returned a core that does not meet minimum_spacing_m; "
            "the core is rejected before publication."
        )
