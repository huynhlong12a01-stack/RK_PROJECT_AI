...
agent_require_package <- function(pkg) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    stop(paste0("Required R package is missing: ", pkg, ". Run .\\_UNG_DUNG\\tools\\check_dependencies.ps1 -Profile core,sampling_design,tidy_io and .\\_UNG_DUNG\\tools\\install_dependencies.ps1 before running this workflow."), call. = FALSE)
  }
  invisible(TRUE)
}
